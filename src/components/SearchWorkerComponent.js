...
export const SearchWorkerComponent = () => {
		...
		_data.forEach(d => {
			...
			if (!_data.filter(_d => _d.selected).map(_d => _d.id).includes(d.id)) {
				...
				if (!exactMatching) {
					...
					matches = fuse.search(adaptedSearchTerm)
						.filter(m => m.score < matchingSharedThresholdWeight)

				} else {
					const attributeValue = (entity, attr) => {
						return (entity[attr] ?? '').toString().toLowerCase().trim();
					}

					const findMatchingItems = (items, searchItem, attributes) => {
						return items.filter(item =>
							attributes.every(attr => attributeValue(item, attr) ===  attributeValue(searchItem, attr))
						);
					}

					matches = findMatchingItems(_data, d, selectedSharedAttr.map(a => a.id))
						.map(match => {
							const item = {
								item: match,
								score: 1
							};
							item.id = match.trackedEntity;

							return item;
						})
				}
				//Filter out ignore data
				matches = matches.filter(m => d.sher1dupli1 ? d.sher1dupli1.indexOf(m.item.id) < 0 : true)
	...
	return (
		<div className={classes.searchResultPage}>
										              ...
										              <div
											              style={{backgroundColor: 'lightblue', margin: 8, padding: 4}}>
                                                            ...
                                                            <span
	                                                            ...
	                                                            onClick={() => {
		                                                            ActionTEI(instance.trackedEntity, 'Open', instance.orgUnit)
	                                                            }}
	                                                            style={{
		                                                            marginLeft: '15px',
		                                                            cursor: selectedRowDeleted.some(item => item === instance.trackedEntity) ? 'not-allowed' : 'pointer'
	                                                            }}>
                                                                <Tag
	                                                                icon={<IconLaunch16/>}
                                                                >
											              ...
											              <span
												              className={classes.customImageContainer}
												              onClick={() => {
													              ActionTEI(instance.trackedEntity, 'Delete', instance.orgUnit)
												              }}
												              style={{
													              marginLeft: '5px',
													              color: 'red',
											              ...
											              <DataTable scrollHeight={scrollHeight}
												              ...
												              <TableHead large={true}>
													              <DataTableRow>
														              <DataTableColumnHeader fixed
														                                     top="0"> OrgUnit </DataTableColumnHeader>
														              ...
														              <DataTableColumnHeader fixed
															              ...
															              <span style={{
																              marginLeft: '2px',
																              fontStyle: 'italic'
															              }}> {i18n.t('lower score represents stronger match')}</span></DataTableColumnHeader>
												              ...
												              <DataTableBody>
													              ...
													              {instance.matches.length > 0 && instance.matches.map(match => (
														              <DataTableRow id={match.trackedEntity}
														                            key={match.trackedEntity}
														                            className={(selectedRowDeleted.some(item => item === match.trackedEntity) ? ` ${classes.deletedItem}` : '')}>
															              ...
															              <DataTableCell>
																              ...
																              <span
																	              ...
																	              style={{
																		              marginLeft: '15px',
																		              cursor: selectedRowDeleted.some(item => item === match.trackedEntity) ? 'not-allowed' : 'pointer'
																	              }}
									...
									>
										...
										{selectedSharedAttr.length > 0 && selectedSharedAttr.map((header, index) => {
											...
											return (
												...
												<DataTableCell id={instance.trackedEntity} key={header.id}
												               className={(selectedRowDeleted.some(item => item === instance.trackedEntity) ? ` ${classes.deletedItem}` : '')}>
													{attribute ? attribute.value : ''}
													{/* {index === 0 && (
                                                        <>
                                                            <span
			...
			{mergeAction &&
				<MergeComponent setMergeAction={setMergeAction} selectedMergingItems={selectedMergingItems}/>}
			{IgnoreAction && <TrackEntityData teiUpdate={teiUpdate} trackedEntityType={trackedEntityType}
			                                  setIgnoreAction={setIgnoreAction}/>}
		</div>
	)
}
